...
export async function GET(req: NextRequest) {
  ...
  const days =
    Number.isInteger(rawDays) && rawDays > 0 ? Math.min(rawDays, 60) : 15;
  ...
  if (error) {
    console.error("Error leyendo historial de avance video:", error);
    return NextResponse.json(
      { error: "Error al obtener el historial de avance video" },
      { status: 500 }
    );
  }

  const rows = ((data ?? []) as DailyWhatsappStatusLog[])
    .map((row) => normalizeWhatsappStatusRow(row))
    .filter((row): row is DailyWhatsappStatusLog => row !== null);
