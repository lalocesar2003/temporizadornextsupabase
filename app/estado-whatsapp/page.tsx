...
function statusButtonClasses(current: WhatsappStatus, target: WhatsappStatus) {
...
export default function EstadoWhatsappPage() {
  ...
  const loadDays = async () => {
    ...
    try {
      ...
      console.error("Error cargando whatsapp status:", error);
      setPageError("No se pudo cargar la bitacora de estados de WhatsApp.");
      setDays([]);
      setRows({});
    } finally {
      setLoading(false);
  ...
  const saveRow = async (date: string, status: WhatsappStatus) => {
    setRows((prev) => ({
      ...prev,
      [date]: {
        ...prev[date],
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div className="space-y-3">
              ...
              <p className="max-w-2xl text-sm leading-6 text-slate-400 md:text-base">
                Marca si subiste o no un estado a tu perfil de WhatsApp. La pagina
                guarda un solo check diario y te deja revisar los ultimos 15 dias.
              </p>
            </div>
          ...
          <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-slate-400">
                Cada fila representa un unico check diario: ¿subiste o no un estado
                a tu perfil de WhatsApp?
              </p>
            </div>
                      ...
                      {row.status === "completed" ? (
                        <>
                          ...
                          <span className="text-emerald-300">
                            Ese dia si subiste un estado a WhatsApp.
                          </span>
                        </>
                      ) : row.status === "failed" ? (
                        <>
